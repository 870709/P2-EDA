...
*/
template<typename ident, typename val> void anadirDependiente(colecInterdep<ident, val>& c, const ident& id, const val& v, const ident& super);
...
template<typename ident, typename val> void anadirRecColateral(typename colecInterdep<ident,val>::Nodo *&p, typename colecInterdep<ident,val>::Nodo *&ptrSuper, const ident& id, const val& v, bool& anadido, const ident& super, const bool& hijoMenor);

template<typename ident, typename val> bool borrarMax(typename colecInterdep<ident,val>::Nodo *&p, ident& id, val& v, typename colecInterdep<ident,val>::Nodo identSup, unsigned int numDepend);

template<typename ident, typename val> bool borrarRec(typename colecInterdep<ident,val>::Nodo *&p, const ident& id, bool& borrado); 
...
struct colecInterdep{
  ...
  private:
    ...
    */
    ...
    friend void anadirRecColateral<ident,val>(Nodo *&p, Nodo *&ptrSuper, const ident& id, const val& v, bool& anadido, const ident& super, const bool& hijoMenor);
    
    friend void borrarRec<ident,val>(Nodo *&p, const ident& id, bool& borrado);
    friend void borrarMax<ident,val>(Nodo *&p);
...
/* Crea una colección vacía, sin elementos.
   Inicializa la colección estableciendo el puntero 'raiz' a 'nullptr' y lo mismo con el iterador 'iter' a 'nullptr'. 
...
/* Devuelve true si existe un elemento en la coleccion c con identificador 'id', false en caso contrario.
   ...
   nulo y 'aux->id == id'.
*/
template<typename ident, typename val> bool existe(const colecInterdep<ident, val>& c, const ident& id, typename colecInterdep<ident,val>::Nodo*& ptrId){
...
template<typename ident, typename val> void encontrarRec(typename colecInterdep<ident,val>::Nodo*& p, const ident& id, typename colecInterdep<ident,val>::Nodo*& ptrId){
  if (p == nullptr){
    ptrId = nullptr;
  } else {
    if (id < p->id){
...
void anadirDependiente(colecInterdep<ident, val>& c, const ident& id, const val& v, const ident& super){
  ...
  bool anadido = false;
  if(id < super){
    anadirRecColateralIdMenor<ident,val>(c.raiz, id, v, super, ptrSuper, anadido);
  }else{
    anadirRecColateralIdMayor<ident,val>(c.raiz, id, v, super, ptrSuper, anadido);
  }
  if (anadido) {
    c.numElem++;
  }
...
/*Dividimos la funcion de anadirDependiente en dos funciones, una para cuando id<super y otra para cuando id>super, para que en cada iteracion recursiva no tega que reevaluar la condicion de id < o > super.
Puesto que aunque le pasaramos por referencia el resutado de la comparacion, esta seguira evaluandola en cada iteracion recursiva.*/
template<typename ident, typename val> void anadirRecColateralHijoMenor(typename colecInterdep<ident,val>::Nodo *&p, const ident& id, const val& v, const ident& super, typename colecInterdep<ident,val>::Nodo*& ptrSuper, bool& anadido){
  if (p == nullptr){
    anadido = false;
    return;
  }
  if(id > p->id){
    anadirRecColateralHijoMenor<ident,val>(p->hijoDer, id, v, super, ptrSuper, anadido);
  }else if(id == p->id){
    anadido = false;
  }else{
    if (super < p->id){
      anadirRecColateralHijoMenor<ident,val>(p->hijoIzq, id, v, super, ptrSuper, anadido);
    }
    else if (super > p->id){
      encontrarRec<ident,val>(p->hijoDer, super, ptrSuper);
      if(ptrSuper!=nullptr){
        anadirRec<ident, val>(p->hijoIzq, id, v, anadido, ptrSuper);
        if(anadido){
          ptrSuper->numDepend++;
...
template<typename ident, typename val> void anadirRecColateralHijoMayor(typename colecInterdep<ident,val>::Nodo *&p, const ident& id, const val& v, const ident& super, typename colecInterdep<ident,val>::Nodo*& ptrSuper, bool& anadido){
  if (p == nullptr){
    anadido = false;
    return;
  }
  if(id < p->id){
    anadirRecColateralHijoMayor<ident,val>(p->hijoIzq, id, v, super, ptrSuper, anadido);
  }else if(id == p->id){
    anadido = false;
  }else{
    if (super > p->id){
      anadirRecColateralHijoMayor<ident,val>(p->hijoDer, id, v, super, ptrSuper, anadido);
    }
    else if (super < p->id){
      encontrarRec<ident,val>(p->hijoIzq, super, ptrSuper);
      if(ptrSuper!=nullptr){
        anadirRec<ident, val>(p->hijoDer, id, v, anadido, ptrSuper);
        if(anadido){
          ptrSuper->numDepend++;
...
template<typename ident, typename val> void borrar(colecInterdep<ident, val>& c, const ident& id){
  bool borrado= false;
...
template<typename ident, typename val> void borrarRec(typename colecInterdep<ident,val>::Nodo *&p, const ident& id, bool& borrado){
  if (p == nullptr){
    borrado = false;
  } else {
    if (id < p->id){
      ...
      if(p->hijoIzq == nullptr){
        ...
        borrarMax<ident,val>(p->hijoIzq, p->id, p->valor, p->identSup, p->numDepend);
      }
    }
  }
}
...
template<typename ident, typename val> void borrarMax(typename colecInterdep<ident,val>::Nodo *&p, ident& id, val& v, typename colecInterdep<ident,val>::Nodo*& identSup, unsigned int& numDepend){
