...
template<typename ident, typename val> void encontrarRec(typename colecInterdep<ident,val>::Nodo* p, const ident& id, bool& encontrado, typename colecInterdep<ident,val>::Nodo*& ptrId){
  if (p == nullptr){
    ...
    if (id < p->id){
      encontrarRec(p->hijoIzq, id, encontrado, ptrId);
    } else if (id == p->id ){
      encontrado = true; 
      ptrId = p;
    } else if (id > p->id){
      encontrarRec(p->hijoDer, id, encontrado, ptrId);
    }
  }
}
...
template<typename ident, typename val> void anadirIndependiente(colecInterdep<ident, val>& c, const ident& id, const val& v){
  bool anadido;
  anadirRec(c.raiz, id, v, anadido);
  if(anadido){
    c.numElem++;
  }
}

template<typename ident, typename val> void anadirRec(typename colecInterdep<ident,val>::Nodo* p, const ident& id, const val& v, bool& anadido){
  if (p == nullptr){
      ...
      if(id < p->id) {
        anadirRec(p->hijoIzq, id, anadido);
      } else if(id == p-> id) {
        anadido = false;
      } else if (id > p->id){
        anadirRec(p->hijoDer, id, anadido);
      }
    }
}
